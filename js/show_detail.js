const movieData = [
  ...
  },
  {
    ...
    rating: '⭐ 4.3 / 5',
    comment: '청소년들의 복잡한 감정을 잘 표현한 영화.',
    description: '파수꾼은 대한민국의 청소년 문제와 인간 관계의 복잡성을 탐구한 드라마 영화입니다.',
  ...
  },
  {
    ...
    rating: '⭐ 4.8 / 5',
    comment: '독창적인 스토리와 시각적 혁신이 돋보이는 영화.',
    description: '에브리씽 에브리웨어 올 앳 원스는 다중 우주와 가족 관계에 대한 독특한 접근을 통해 관객들에게 놀라움을 선사합니다.',
  ...
  },
  {
    ...
    rating: '⭐ 4.5 / 5',
    comment: '실화를 바탕으로 한 긴장감 넘치는 이야기.',
    description: '모가디슈는 소말리아 내전 중에 고립된 대한민국 대사관의 탈출기를 다룬 실화 영화로, 압도적인 긴장감과 감동을 선사합니다.',
  ...
  },
  {
    ...
    rating: '⭐ 4.4 / 5',
    comment: '대서사시 같은 액션과 감동이 함께하는 영화.',
    description: 'RRR은 인도의 독립 운동을 배경으로 한 역사적 인물들의 우정과 희생을 그린 대작으로, 화려한 액션과 드라마가 돋보입니다.',
  ...
  },
  {
    ...
    actors: '설혜인, 최수인 외',
    rating: '⭐ 4.6 / 5',
    comment: '어린이들의 미묘한 감정을 섬세하게 표현한 영화.',
    description: '우리들은 어린이들의 복잡한 심리와 관계를 진지하게 탐구하며, 공감과 따뜻함을 불러일으키는 영화입니다.',
  ...
  },
  {
    ...
    rating: '⭐ 4.3 / 5',
    comment: '서정적인 이야기가 돋보이는 일본 영화.',
    description: '굿 앤 바이는 인간 관계의 이별과 감정의 복잡성을 잔잔하게 그려내며, 서정적인 분위기 속에서 감동을 전해줍니다.',
...
function updateDetailContent(index) {
  ...
  document.getElementById('summary').innerHTML = movie.summary; // innerHTML 사용
  const gotoseeBtn = document.getElementById('gotosee');

  // 링크를 새 탭에서 열거나 알럿을 띄우기 위한 onclick 이벤트 설정
  if (movie.link === 'None') {
...
document.querySelector('.detail-btn .button--bubble').addEventListener('click', function() {
  const detailContainer = document.getElementById('modalOverlay');
  
  if (!isDetailOpen) {
    stopSlideInterval(); // 슬라이드 멈추기
    updateDetailContent(currentIndex); // 현재 슬라이드 인덱스에 맞는 영화 정보 업데이트
    detailContainer.classList.add('show'); // 모달 열기
    
    // 슬라이드 애니메이션 상태 플래그를 멈추도록 설정
...
document.getElementById('closeModal').addEventListener('click', function() {
  const detailContainer = document.getElementById('modalOverlay');
  detailContainer.classList.remove('show'); // 모달 닫기

  startSlideInterval(); // 슬라이드 재시작
  isDetailOpen = false;
...
document.getElementById('modalOverlay').addEventListener('click', function(event) {
  if (event.target === event.currentTarget) { // 오버레이를 클릭한 경우에만 닫기
